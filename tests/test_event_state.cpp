...
TEST_CASE("EventState: begin_frame resets keys and mouse buttons", "[event_state]") {
    EventState es;
    ...
    CHECK_FALSE(es.was_key_released(SDL_SCANCODE_X));
    CHECK_FALSE(es.mouse_button_down(SDL_BUTTON_LEFT));
    CHECK(es.mouse_rel_x == 100.0f); // public fields NOT reset by begin_frame
}
